"""
Node Health Collector Module
Collects and analyzes node health metrics for different node groups (master/worker/infra/workload)
"""
...
class nodeHealthCollector:
    """Collector for node health metrics with node group support"""
    ...
    async def collect_all_metrics(self, node_group: str = 'master', duration: Optional[str] = None, 
        ...
        """Collect all node health metrics for specified node group
        ...
        """
        try:
            self.logger.info(f"Starting node health metrics collection for group: {node_group}")
            ...
            async with PrometheusBaseQuery(prom_config['url'], self._prom_token) as prom:
                ...
                pleg_relist = await self._collect_p99_kubelet_pleg_relist_duration(
                    prom, nodes, start_str, end_str, node_name_map
                )
            ...
            self.logger.error(f"Error collecting node health metrics: {e}", exc_info=True)
            return {
                'status': 'error',
                'error': str(e),
                'timestamp': datetime.now(timezone.utc).isoformat()
    ...
    async def _collect_p99_kubelet_pleg_relist_duration(self, prom: PrometheusBaseQuery, 
                                                        nodes: List[str], 
                                                        start: str, end: str,
...
async def main():
    ...
    print("\n" + "="*80)
    print("Node Health Collector - PLEG Relist Latency Monitoring")
    print("="*80)
    print("\nFeatures:")
    ...
    print("  ✓ Return top N nodes by PLEG relist latency")
    ...
    print("    High values indicate kubelet performance issues")
    print("\nUsage Examples:")
    print("  # Query master nodes for last hour")
    print("  collector = nodeHealthCollector(ocp_auth, prometheus_config)")
    ...
    print("  result = await collector.collect_all_metrics(node_group='worker', duration='30m', top_n_nodes=3)")
    print()
    ...
    print("    ],")
    print("    'metrics': {")
    print("      'p99_kubelet_pleg_relist_duration': {")
    print("        'nodes': {")
    print("          'node-1.example.com': {")
    print("            'p99': 0.0125,  # seconds")
    print("            'max': 0.0150,")
    print("            'min': 0.0100,")
    print("            'unit': 'second'")
    print("          }")
    ...
    print("      }")
    print("    },")
    print("    'top_nodes_by_pleg_latency': {")
    print("      'count': 3,")
    print("      'sort_by': 'p99_latency',")
    print("      'nodes': [")
    print("        {")
    print("          'rank': 1,")
    print("          'node': 'node-3.example.com',")
    print("          'role': 'worker',")
    print("          'p99_latency': 0.0250,")
    print("          'unit': 'second'")
    print("        }")
    print("      ]")
    print("    }")
    ...
    print("  • Critical: > 1s")
    print("  High PLEG latency can cause:")
    print("    - Slow pod lifecycle operations")
    print("    - Delayed pod status updates")
    print("    - Node performance degradation")
    print("="*80 + "\n")
